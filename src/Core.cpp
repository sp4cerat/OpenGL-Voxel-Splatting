...
int CheckGLError(char *file, int line)
{
	...
	{
		printf("GL error at line %d file %s\n\n",line,file);
...
void CoreKeyMouse()
{
	...
	if ( keyboard.KeyPr(13) ) CoreToggleFullscreen();

	keyboard.update();
	mouse.update();

	static int time = timeGetTime (); int timeNew = timeGetTime ();
	float deltaTime = float(int(timeNew-time)); time=timeNew;

	screen.screentime=(screen.screentime*0.9)+(deltaTime*0.001)*0.1;
	...
	if ( key[GLUT_KEY_PAGE_UP   ] ) screen.pos=screen.pos-movY * (0.03 * deltaTime);
	if ( key[GLUT_KEY_PAGE_DOWN ] ) screen.pos=screen.pos+movY * (0.03 * deltaTime);
	...
	if ( mouse.button[2] )
	{
		screen.fov *= (1.0+mouse.mouseDY);
		if(screen.fov<10) screen.fov=10;
		if(screen.fov>120) screen.fov=120;
	}
}
////////////////////////////////////////////////////////////////////////////////
